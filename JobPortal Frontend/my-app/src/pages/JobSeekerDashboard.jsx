      ...
      <div className="overlay">
        <div className="container py-5 d-flex justify-content-center">
          <div className="col-lg-10">
            ...
            {jobs.map(job => (
              <div className="card mb-4" key={job.id}>
                <div className="card-body">
                  ...
                  <input
                    type="file"
                    accept=".pdf"
                    className={`form-control my-2 ${errors[job.id] ? 'is-invalid' : ''}`}
                    onChange={e => handleResumeChange(job.id, e.target.files[0])}
                  />
                  {errors[job.id] && <div className="text-danger">{errors[job.id]}</div>}

                  <button className="btn btn-success mt-2" onClick={() => handleApply(job.id)}>Apply</button>
                </div>
              </div>
            ))}
          </div>
